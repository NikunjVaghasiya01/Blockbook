...
// BlocxParser handle
type BlocxParser struct {
	*btc.BitcoinLikeParser
	baseparser *bchain.BaseParser
}

// NewBlocxParser returns new BlocxParser instance
func NewBlocxParser(params *chaincfg.Params, c *btc.Configuration) *BlocxParser {
	return &BlocxParser{
...
// GetChainParams contains network parameters for the main Blocx network,
// the regression test Blocx network, the test Blocx network and
// the simulation test Blocx network, in this order
...
func (p *BlocxParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *BlocxParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
